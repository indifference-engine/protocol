...
void test()
{
  ...
  assert(!read_u8s(actual_buffer, 6, actual_u8s, 3, 5), "read_u8s invalid_2");

  const uint8_t expected_u8s[] = {0x71, 0x51, 0x62};
  assert_u8s(expected_u8s, actual_u8s, 3, "read_u8s invalid_2 u8s");
  const uint8_t expected_buffer[] = {0x08, 0xe5, 0x2b, 0x39, 0xcb, 0x1b};
  assert_u8s(expected_buffer, actual_buffer, 6, "read_u8s invalid_2 buffer");
}
