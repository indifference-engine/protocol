...
void test()
{
  ...
  assert(!read_s16s(actual_buffer, 12, actual_s16s, 3, 8), "read_s16s invalid_2");

  const int16_t expected_s16s[] = {0x7151, 0x62FA, -0x1FE8};
  assert_s16s(expected_s16s, actual_s16s, 3, "read_s16s invalid_2 s16s");
  const uint8_t expected_buffer[] = {0x0F, 0x08, 0xe5, 0x2b, 0x39, 0xcb, 0x1b, 0x85, 0x13, 0x31, 0x89, 0x10};
  assert_u8s(expected_buffer, actual_buffer, 12, "read_s16s invalid_2 buffer");
}
